...
import { Bug, Rocket, Server, TrendingUp, TrendingDown, Target, Calendar, Loader2 } from 'lucide-react';
...
export default function EngineeringTab() {
  ...
  const dataTableColumns = [
    ...
    { id: 'featuresShipped', label: 'Features', editable: true },
    { id: 'openBugs', label: 'Open Bugs', editable: true },
    { id: 'uptime', label: 'Uptime %', editable: true, format: (v) => `${(v || 0).toFixed(2)}%` },
  ];
  ...
  const metricConfigs = [
    { id: 'featuresShipped', title: 'Features Shipped', color: '#10B981', bgColor: '#D1FAE5', icon: Rocket, format: (v) => v, isLowerBetter: false },
    { id: 'openBugs', title: 'Open Bugs', color: '#EF4444', bgColor: '#FEE2E2', icon: Bug, format: (v) => v, isLowerBetter: true },
    { id: 'uptime', title: 'Uptime', color: '#3B82F6', bgColor: '#DBEAFE', icon: Server, format: (v) => `${(v || 0).toFixed(2)}%`, isLowerBetter: false },
  ];
  ...
  const getProgressPercent = (value, goal, isLowerBetter) => {
    if (!goal) return 0;
    if (isLowerBetter) return Math.min((goal / Math.max(value, 1)) * 100, 100);
    return Math.min((value / goal) * 100, 100);
  };

  const getProgressColor = (value, goal, isLowerBetter) => {
    const percent = isLowerBetter ? (goal / Math.max(value, 1)) * 100 : (value / goal) * 100;
        ...
        {metricConfigs.map(config => {
          ...
          return (
            <div key={config.id} style={styles.metricCard}>
              ...
              <div style={{ ...styles.metricValue, color: config.color }}>{config.format(value)}</div>
              <div style={styles.metricGoal}>
                <Target size={14} />
                <span>Goal: {config.isLowerBetter ? '≤' : '≥'} {config.format(goal)}</span>
